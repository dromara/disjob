...
CREATE TABLE `sched_job` (
    ...
    `job_type`             tinyint(3)    unsigned  NOT NULL DEFAULT '1'                                           COMMENT 'Job类型：1-普通(Normal)；2-广播(Broadcast)；3-工作流(Workflow)；4-分布式计算(MapReduce)；',
...
CREATE TABLE `sched_task` (
    ...
    `execute_state`        tinyint(3)    unsigned  NOT NULL                                                       COMMENT '执行状态：10-等待执行；20-正在执行；30-暂停执行；40-正常完成；50-实例化失败取消；51-校验失败取消；52-初始化异常取消；53-执行失败取消；54-执行异常取消；55-执行超时取消；56-执行冲突取消(sched_job.collision_strategy=3)；57-手动取消；58-广播任务分配worker后因死亡导致未执行取消；',
...
CREATE TABLE `workflow_instance` (
    `id`                   bigint(20)    unsigned  NOT NULL AUTO_INCREMENT                                        COMMENT '自增主键ID',
    `parent_instance_id`   bigint(20)    unsigned  NOT NULL                                                       COMMENT 'sched_instance.instance_id',
    ...
    `created_at`           datetime                NOT NULL DEFAULT CURRENT_TIMESTAMP                             COMMENT '创建时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_parentinstanceid_prenodeid_curnodeid` (`parent_instance_id`, `pre_node_id`, `cur_node_id`),
    KEY `ix_createdat` (`created_at`),
    KEY `ix_updatedat` (`updated_at`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='workflow实例表';
...
-- ----------------------------
INSERT INTO `sched_job` (`job_id`, `job_group`, `job_name`, `job_handler`, `job_state`, `job_type`, `job_param`, `trigger_type`, `trigger_value`, `next_trigger_time`) VALUES (3988904755200, 'default', 'noop-job',      'cn.ponfee.scheduler.core.handle.impl.NoopJobHandler',             1, 1, '',                                                                    1, '0/5 * * * * ?',       unix_timestamp()*1000);
INSERT INTO `sched_job` (`job_id`, `job_group`, `job_name`, `job_handler`, `job_state`, `job_type`, `job_param`, `trigger_type`, `trigger_value`, `next_trigger_time`) VALUES (3988904755300, 'default', 'http-job',      'cn.ponfee.scheduler.core.handle.impl.HttpJobHandler',             1, 1, '{"method":"GET", "url":"https://www.baidu.com"}',                     1, '0/10 * * * * ?',      unix_timestamp()*1000);
INSERT INTO `sched_job` (`job_id`, `job_group`, `job_name`, `job_handler`, `job_state`, `job_type`, `job_param`, `trigger_type`, `trigger_value`, `next_trigger_time`) VALUES (3988904755400, 'default', 'command-job',   'cn.ponfee.scheduler.core.handle.impl.CommandJobHandler',          1, 1, '{"cmdarray":["/bin/sh","-c","echo $(date +%Y/%m/%d)"]}',              1, '0/15 * * * * ?',      unix_timestamp()*1000);
INSERT INTO `sched_job` (`job_id`, `job_group`, `job_name`, `job_handler`, `job_state`, `job_type`, `job_param`, `trigger_type`, `trigger_value`, `next_trigger_time`) VALUES (3988904755500, 'default', 'script-job',    'cn.ponfee.scheduler.core.handle.impl.ScriptJobHandler',           1, 1, '{"type":"SHELL","script":"#!/bin/sh\\necho \\"hello, shell!\\"\\n"}', 1, '0/10 * * * * ?',      unix_timestamp()*1000);
INSERT INTO `sched_job` (`job_id`, `job_group`, `job_name`, `job_handler`, `job_state`, `job_type`, `job_param`, `trigger_type`, `trigger_value`, `next_trigger_time`) VALUES (4236701614080, 'default', 'prime-count',   'cn.ponfee.scheduler.samples.common.handler.PrimeCountJobHandler', 1, 1, '{\"m\":1,\"n\":6000000000,\"blockSize\":100000000,\"parallel\":7}',   2, '2022-10-06 22:53:00', unix_timestamp()*1000);
INSERT INTO `sched_job` (`job_id`, `job_group`, `job_name`, `job_handler`, `job_state`, `job_type`, `job_param`, `trigger_type`, `trigger_value`, `next_trigger_time`) VALUES (4236701615230, 'default', 'broadcast-job', 'cn.ponfee.scheduler.core.handle.impl.NoopJobHandler',             1, 2, '',                                                                    2, '2023-03-18 21:30:00', unix_timestamp()*1000);
