...
@Getter
@Setter
@ConfigurationProperties(prefix = JobConstants.WORKER_CONFIG_KEY)
public class WorkerProperties extends ToJsonString implements Serializable {
    ...
    public void check() {
        ...
        Assert.isTrue(0 < maximumPoolSize && maximumPoolSize <= ThreadPoolExecutors.MAX_CAP, "Maximum pool size must be range [1, 32767].");
