...
CREATE TABLE `sched_job` (
  ...
  `retry_interval`        INT            UNSIGNED  NOT NULL  DEFAULT '0'                  COMMENT '实例失败的重试间隔(毫秒)，阶梯递增(retry_interval * retry_attempt^2)',
  ...
  `alert_options`         TINYINT        UNSIGNED  NOT NULL  DEFAULT '0'                  COMMENT '告警选项Bitwise(位的或运算结果)：1-警报；2-通知；3-全选；',
  ...
  `scan_failures`         TINYINT        UNSIGNED  NOT NULL  DEFAULT '0'                  COMMENT '连续扫描失败的次数，连续失败次数达到阈值后自动禁用',
...
CREATE TABLE `sched_instance` (
  ...
  `retry_attempt`         TINYINT        UNSIGNED  NOT NULL  DEFAULT '0'                  COMMENT '当前是第几次重试(最大值为retry_count)',
  `next_scan_time`        DATETIME(3)              NOT NULL  DEFAULT CURRENT_TIMESTAMP(3) COMMENT '下一次的扫描时间',
...
CREATE TABLE `sched_task` (
  ...
  `execution_data`        TEXT                               DEFAULT NULL                 COMMENT '保存的执行数据',
  `worker`                VARCHAR(255)                       DEFAULT NULL                 COMMENT '工作进程(JVM进程，GROUP:WORKER-ID:HOST:PORT)',
  `idempotency_key`       VARCHAR(32)                        DEFAULT NULL                 COMMENT '启动任务时的幂等键，用于start请求超时重试幂等',
