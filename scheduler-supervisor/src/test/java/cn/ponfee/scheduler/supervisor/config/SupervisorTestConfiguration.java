 ...
 */
@Configuration
public class SupervisorTestConfiguration {
    ...
    @DependsOn(SupervisorConstants.SPRING_BEAN_NAME_CURRENT_SUPERVISOR)
    @Bean
    public ScanTrackHeartbeatThread scanTrackHeartbeatThread(@Value("${" + DISTRIBUTED_SCHEDULER_SUPERVISOR + ".log.heartbeat-interval-seconds:3}") int heartbeatIntervalSeconds,
                                                             JobManager jobManager,
                                                             @Qualifier(DB_NAME + Constants.JDBC_TEMPLATE_SUFFIX) JdbcTemplate jdbcTemplate) {
